...
kthread_create(void (*func)(void*), void *arg_ptr, void *tstack)
{
  ...
  if(curproc->isThread){         // if curproc is a thread, set np's parent to its parent process
    np->parent = curproc->parent;
    np->parent->threadCount++;
  }
  else if(!curproc->isThread){  // if curproc is a process, make it np's parent
    np->parent = curproc;
    curproc->isParent = 1;
    curproc->threadCount++;
  }

  *np->tf = *curproc->tf;                   // copy trap frame from curproc
  ...
  *((int *) (np->tf->esp)) = (int) arg_ptr; // push arg_ptr onto the stack
  ...
  tid = np->parent->nextTid;        // assign tid
  np->tid = tid;                    // assign np's unique thread ID
  np->parent->nextTid++;            // increment parent's nextTid
  np->tf->esp -= sizeof(int);       // make space for an int to be pushed onto the stack
...
kthread_join(int tid)
{
  ...
  if((curproc->isParent && curproc->threadCount == 0) || tid == 0){
    return -1;
  }
...
wait(void)
{
  ...
  for(;;){
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
#ifdef KTHREADS
// You will need to change how wait() kills off processes marked as zombie
// you do not want the kernel deallocating memory for a thread.
// you need to take care of that in kthread_join and benny_thread_exit
// kthread_join will call kfree() and benny_thread_exit will call free
// on the memory allocated for the thread stack
...
#ifdef KTHREADS
// You need to have this data member in the proc struct
      p->oncpu = current_cpu;
#endif // KTHREADS
...
#ifdef KTHREADS
            cprintf("%d\t%u\t%u\t%u"
                    , ptable.proc[i].oncpu >= 0 ? ptable.proc[i].oncpu : '\0'
